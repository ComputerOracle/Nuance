"""Tests for POST /escrows/{id}/fund/on-chain — the ack endpoint
components/app/genlayer-write-client.ts's fundEscrowOnChain calls after
signing and sending a real, payable NuanceEscrow.fund_escrow transaction.

Covers:
  1. Happy path: the escrow creator funding a linked escrow records the
     tx hash.
  2. Only the creator may call it (403 for the counterparty or anyone
     else) — mirrors the contract's own fund_escrow restriction.
  3. Rejects an escrow not yet linked to a deployed contract (400).
  4. Rejects a malformed tx_hash (422, from the schema itself).
"""
...
_TMP_DIR = tempfile.mkdtemp(prefix="nuance-escrow-fund-onchain-test-")
...
async def _create_escrow(creator: str, counterparty: str, contract_address: str | None) -> int:
    async with AsyncSessionLocal() as db:
        ...
        escrow = Escrow(
            ...
            title="Fund on-chain test escrow",
            total=Decimal("2.50"),
...
def test_fund_escrow_on_chain_happy_path(client: TestClient):
    ...
    assert resp.json()["funded_tx_hash"] == _FAKE_TX_HASH


def test_fund_escrow_on_chain_requires_creator(client: TestClient):
    creator = Account.create()
    counterparty = Account.create()
...
def test_fund_escrow_on_chain_rejects_unlinked_escrow(client: TestClient):
    creator = Account.create()
    counterparty = Account.create()
    token = _get_token(client, creator)
    _get_token(client, counterparty)
...
def test_fund_escrow_on_chain_rejects_malformed_hash(client: TestClient):
    creator = Account.create()
    counterparty = Account.create()
    token = _get_token(client, creator)
    _get_token(client, counterparty)
