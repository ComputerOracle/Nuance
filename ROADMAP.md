...
>
> **Real GEN-staked on-chain voting (2026-09-13).** `cast_vote` is now payable (GEN stake = vote weight) and a real `retract_vote` (unvote) exists, both mirroring `bet()`'s own GEN-staking pattern in Prediction Markets — asked directly: "any user that vote and unvote you will have to use Gen token ... like a real Governance." `NuanceGovernance` redeployed for this (`0x37303291cA1438F05b2236b7Dd9667aEeFA8f310`, see §4.4.2's own table). Existing proposals stay `LEGACY_OFFCHAIN` by design; only new proposals go on-chain.
>
> **Autonomous proposal generator (2026-09-13).** `backend/app/services/governance_generator.py` + `governance_ingestion_scheduler.py` — the Governance equivalent of `market_generator.py`/`market_ingestion_scheduler.py` (§5's "fetch update for the API key... every week" request, asked again directly for Governance: "refresh and update the Governance use the API key"). Reuses that module's own ingestion (`_ingest_events`/`RawEvent`/`_is_relevant_candidate`) rather than re-fetching independently; dedup is a genuinely separate `GovernanceEventLog` table so the two generators sharing one event stream never starve each other. Deliberately a much higher extraction bar than predictions' own ("is there a real decision to vote FOR/AGAINST", not just "is this a checkable future milestone") — expected to produce proposals far less often, by design, not as a bug. No responsible offline (no-API-key) fallback exists for this one — see `_extract_proposal_offline`'s own docstring on why fabricating a governance decision without LLM judgment was judged too risky to mirror predictions' own keyword-heuristic fallback. `Proposal.proposer_address` is a real FK into `users` (unlike `Prediction`, which has no such column) — the generator get-or-creates a dedicated system `User` row for this rather than requiring one to be pre-seeded by hand.
...
### 6.2 Multi-Token Collateral — backend ✅ 2026-09-09; frontend selectors not started

> The Build Plan's Emma/Product track lists "Multi-asset selectors in the Create Escrow and Place Bet flows" — as of 2026-09-08 no `Asset` model existed anywhere in this codebase, so there was nothing but `GEN` for a selector to actually offer; building one then would've been either a cosmetic single-option dropdown or a client-side-only list with no backend behind it. That blocker is gone as of this pass — the `Asset` model, `EscrowCreate.asset_symbol`, and `EscrowRead.asset` below are real and tested — but the actual Create-Escrow/Place-Bet UI selector components themselves are still **not built**; that's real frontend work now unblocked, not done.
...
**Status as of 2026-09-09:** §6.1 is fully done — the Real Agent Directory drill-down, API keys, X-Api-Key auth, webhooks, and per-key rate limits, from two parallel passes that happened to land on opposite halves of this section's checklist. §6.2's backend (Asset model, multi-token escrows, widened precision, migration) is done and unblocks the Build Plan's multi-asset-selector item, but that frontend selector UI itself is still not built (see §6.2's own note). §6.3's SDK, backend Sentry, incident runbook, and public docs site are done; the security audit, gas modeling, treasury multi-sig, frontend Sentry, and Grafana/Prometheus remain untouched. Every "done" item above (both from this pass and the parallel one it merged with) has real live/test verification behind it, not just code written — see each bullet.
